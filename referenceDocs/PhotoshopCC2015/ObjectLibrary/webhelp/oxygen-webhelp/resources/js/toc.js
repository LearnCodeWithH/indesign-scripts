...
var searchForm = '<form name="objectSearch" id="objectSearch" action="javascript:void(0)"' + 
    'onsubmit=" doNav(1);">'+
    '<input id="textToSearchInObject" name="textToSearchInObject" class="textToSearch" size="30" placeholder="Search Object" type="text"/></form> ' +
    ...
    'size="30" placeholder="Search API"/>' +
    '</form>';
...
$(document).ready(function () {
    ...
    if (contextId !== undefined && contextId != "") {
        ...
        $.each($('#tree a[data-id*="' + contextId + '"]'), function () {
            ...
            if ($.inArray(contextId, idArray) >= 0) {
                ...
                if (!withFrames) {
                    var newLocation = "";
                    if (p.host != '') {
                        newLocation = p.protocol + '://' + p.host + ':' + p.port + p.path + '#' + $(this).attr('href');
                } else {
                        newLocation = p.protocol + '://' + p.port + p.path + '#' + $(this).attr('href');
                }
        ...
        if (!found && withFrames) {
            $('#tree a[data-id]').first().trigger('click');
        }
    } else {
        if (withFrames) {
            if (q != undefined) {
...
function loadIndexterms() {
    try {
        $('#loadingError').remove();
        if (typeof window.indextermsLoaded == 'undefined') {
            $("#indexBlock #iList").load("./oxygen-webhelp/indexterms.html", 
                function() {
...
$('#text-search').bind('keyup change', function(ev) {
    ...
    $("#frm").contents().find(".frmBody").removeHighlight();
    
    // disable highlighting if empty
    if ( searchTerm ) {
        // highlight the new term
        $("#frm").contents().find(".frmBody").highlight( searchTerm );
        matchArr = $("#frm").contents().find(".highlight").toArray();
    }
});
